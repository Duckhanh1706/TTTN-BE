...
export const createLesson = async (req, res) => {
  try {
    ...
    if (!title || !courseId) {
      return res
        .status(400)
        .json({ message: "Thiếu thông tin tiêu đề hoặc mã khóa học!" });
    }

    const videoUrl = req.file ? `/uploads/${req.file.filename}` : null;

    const [result] = await db.query(
      "INSERT INTO lessons (course_id, title, video_url, content, position) VALUES (?, ?, ?, ?, ?)",
      [courseId, title, videoUrl, content || "", position || 0],
    );

    return res.status(201).json({
      message: "Tạo bài học và tải lên video thành công!",
      data: {
        id: result.insertId,
        course_id: courseId,
        title,
    ...
    console.error("Lỗi khi tạo bài học:", error.message);
    return res
      .status(500)
      .json({ message: "Lỗi Server nội bộ", error: error.message });
  }
...
export const getCommentsByLesson = async (req, res) => {
  try {
